...
package body Test_Traverses is
   ...
   is
      ...
      Log_File  : constant String :=
        Root & "/obj/test_traverses_global_build.log";
   ...
   begin
      ...
      begin
         ...
         --  A Suspension_Object local to a (non-terminating) task's own
         --  first declarative section, and one local to main's own first
         --  declarative section, are both found -- neither is a task or
         --  protected declaration itself, so neither is restricted by
         --  Ravenscar's No_Task_Hierarchy/No_Local_Protected_Objects.
         Op.Assert (Any_Contains ("Priority_Sample.Telemetry.Local_Ready"));
         Op.Assert (Any_Contains ("Priority.Main_Ready"));

         --  Proper library-level names (already covered by
         --  Each_Library_Level_Name) are never re-yielded here -- only
         --  what's local to main/a task's own first declarative section.
         Op.Assert (not Any_Contains ("Priority_Sample.Guard"));
         Op.Assert (not Any_Contains ("Priority_Sample.Ready"));
      end;
   end Test_Each_Effectively_Global_Name;
