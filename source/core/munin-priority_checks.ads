...
--  graph exposed through Munin.Call_Graph_Providers: for every task and
--  interrupt handler, tracks the active priority it runs at (raised to a
--  protected object's ceiling on entry, per D.3(6)) as it walks the
--  object's operations, and reports every protected operation reachable
--  at a priority higher than its object's ceiling -- exactly the
--  condition that raises Program_Error at run time.
...
package Munin.Priority_Checks is
   ...
   type Violation is record
      ...
      --  is a task or interrupt-handler root (an element of some
      --  Call_Graph_Provider's Tasks or Interrupt_Handlers), Path
      --  (Path.Last_Index) is the violating protected-operation node
   ...
   --  Tasks or Interrupt_Handlers, one per violating call site (regardless
   --  of how many distinct call paths reach it), each carrying the
   --  worst-case active priority and one example path that reaches it.
   --  Empty when no violation is reachable.
