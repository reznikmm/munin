...
--  Call_Graph_Provider interface (Callees, Tasks, Interrupt_Handlers);
--  provider-agnostic, so it works unchanged for any current or future
--  Call_Graph_Provider implementation.
...
package Munin.Call_Graph_Cycles is
   ...
   --  Provider.Tasks or Provider.Interrupt_Handlers: each strongly
   --  connected component of Provider's call graph with more than one
   --  node, plus every single node with a direct self-loop. A node not
   --  reachable from either root set is never visited, so a cycle
   --  entirely among such nodes (e.g. dead code) is not reported --
   --  consistent with Munin.CLI.Main's call-tree printing, which is
   --  rooted the same way. Uses Tarjan's algorithm (a single pass over
   --  Callees; no separate reverse-graph pass, unlike Kosaraju's). Empty
   --  when Provider's call graph, restricted to what's reachable from
   --  those roots, is acyclic.
